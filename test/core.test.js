...
test('parseReceiptPayload throws on invalid payload', () => {
  assert.throws(
    () => parseReceiptPayload('{"merchant":"Walmart"}'),
...
test('predictWeeklyNeeds returns empty when stock covers demand', () => {
  const pantry = [{ name: 'Rice', unit: 'kg', quantity: 5 }];
  const usage = [{ name: 'Rice', unit: 'kg', amount: 2 }];
